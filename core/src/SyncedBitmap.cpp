...
void SyncedBitmap::SetBit(int bit_index, bool val) {
    assert(bit_index < _size * 8);

    std::lock_guard<std::mutex> l(_bitmap_mutex);

    if(val)
        _bytes[(bit_index - 1) / 8] |= (1 << ((bit_index - 1) % 8));
    else
        _bytes[(bit_index - 1) / 8] &= ~(1 << ((bit_index - 1) % 8));
}

bool SyncedBitmap::GetBit(int bit_index)  {
    std::lock_guard<std::mutex> l(_bitmap_mutex);
    return (bool)((1 << ((bit_index - 1) % 8)) & (_bytes[(bit_index - 1) / 8]));
}
